...
function draw() {
  background(sp);  
    ...
    if (gameState === "serve" && keyDown("Q")) {
      
      fill("WHITE")
      text("THIS IS A WORLD WHERE SPACE SHOOT IS HELD YOU HAVE KILL ALL THE FIGHTER PLANES ",20,300);
      text("WHICH ARE COMING TO HIT YOU BE AWARE DONT GET HIT BY THE FIGHTER PLANE!!!",35,320)
      text("PRESS Q TO START THE GAME",200,350)
     serve()
      gameState = PLAY;
...
if(bullet1.isTouching(obstaclesGroup)){
  obstacle.destroy()
}
if(sc.isTouching(obstaclesGroup)){
  sc.velocityX = 0;
  sc.velocityY = 0;
